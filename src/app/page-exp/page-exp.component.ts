...
export class PageExpComponent implements OnInit {
  ...
  constructor(
    ...
    this.experiences = {
      ...GlobalStrings.projects,
      ...GlobalStrings['work exp'],
    };
  ...
  filerExp(type: string, $event) {
    ...
    if (this.showJobs === this.showProjs) {
      message = this.showJobs
        ? GlobalStrings.toasts.both
        : GlobalStrings.toasts.none;
    } else {
      message = this.showJobs
        ? GlobalStrings.toasts.work
        : GlobalStrings.toasts.projects;
    }
    if (message != '') {
  ...
  keyPipeSort(a, b) {
    const a_time = a.value.time[0].split('-')[0].trim();
    const b_time = b.value.time[0].split('-')[0].trim();
